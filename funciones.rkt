...
(define (completeKey llave txt)
	...
	(define (ciclo)
		(cond
		[(< cont (string-length txt))
		(cond ((= cont2 (string-length llave)) (set! cont2 0)))
                (cond 
...
;(define name-txt "")
 ;   (cond 
   ;     [(file-exists? name)
  ;          (set! name-txt (string-append name "-1"))
 ;       ]
 ;   )
 ;   (define ou (out))

;    (define oFile (open-output-file ))

;)
(define (get-file-text file)
    (define txtResult "")
    (define iFile (open-input-file file))
    (cond
        [(file-exists? file)
